...
public class NutriologoValidar implements Validator {
    ...
    public void validate(Object o, Errors errors) {
        ...
        if (!nutriologo.getCorreo().equals("")) {

            if (nutriologo.getCorreo().charAt(nutriologo.getCorreo().length() - 1) == '.' || nutriologo.getCorreo().charAt(nutriologo.getCorreo().length() - 1) == '.') {
                errors.rejectValue("correo", "correo.incorrect", "El correo no es valido");
            } else {
                int arroba = 0;
                int postArroba = 0;
                int comillaSimple = 0;
                for (int i = 0; i < nutriologo.getCorreo().length(); i++) {
                    if (nutriologo.getCorreo().charAt(i) == '@') {
                        arroba = 1;
                    }
                    if (nutriologo.getCorreo().charAt(i) == '.' && arroba == 1) {
                        postArroba = 1;
                    }
                ...
                if (postArroba == 0) {
                    ...
                    if (nutriologo.getCorreo().matches("['*$#%&/!?¡¿{}+*~|^\\']")) {
                        errors.rejectValue("correo", "correo.incorrect", "El correo no es valido");
                    } else {
        ...
        if (!nutriologo.getNo_empleado().equals("")) {

            if (nutriologo.getNo_empleado().matches("\\d{10}")) { // valida que se ingresen solo números
                System.out.println("SOLO TIENE NUMEROS EL NUMERO DE EMPLEADO");
                System.out.println(nutriologo.getNo_empleado().length());
            } else {
        ...
        if (!nutriologo.getNo_cedula().equals("")) {

            if (nutriologo.getNo_cedula().matches("\\d{10}") || nutriologo.getNo_cedula().matches("\\d{7}") || nutriologo.getNo_cedula().matches("\\d{8}") || nutriologo.getNo_cedula().matches("\\d{9}")) { // valida que se ingresen solo números
        ...
        sql = " select telefono from administrador";   // CONSULTA 
        datosL2 = this.jdbcTemplate.queryForList(sql);                                  //ASIGNACIÓN DE RESULTADO DE CONSULTA
        ...
        sql = " select telefono from nutriologo";   // CONSULTA 
        datosL2 = this.jdbcTemplate.queryForList(sql);                                  //ASIGNACIÓN DE RESULTADO DE CONSULTA
        ...
        sql = " select telefono from psicologo";   // CONSULTA
        datosL2 = this.jdbcTemplate.queryForList(sql);                                  //ASIGNACIÓN DE RESULTADO DE CONSULTA
        ...
        sql = " select telefono from paciente";   // CONSULTA 
        datosL2 = this.jdbcTemplate.queryForList(sql);                                  //ASIGNACIÓN DE RESULTADO DE CONSULTA
        ...
        ////////////////VALIDACION CEDULA
        sql = " select no_cedula from nutriologo";   // CONSULTA 
        datosL2 = this.jdbcTemplate.queryForList(sql);                                  //ASIGNACIÓN DE RESULTADO DE CONSULTA
        ...
        sql = " select no_cedula from psicologo";   // CONSULTA 
        datosL2 = this.jdbcTemplate.queryForList(sql);                                  //ASIGNACIÓN DE RESULTADO DE CONSULTA
        ...
        if (!nutriologo.getHoraEntrada().equals("")) {
            if (!nutriologo.getHoraSalida().equals("")) {
                ...
                System.out.println("otra hora inicio " + inicio);
                System.out.println("otra hora fin " + fin);
                DateFormat dateFormat = new SimpleDateFormat("HH:mm");
                Date horaIni;
                Date horaFin;
                try {
                    horaIni = dateFormat.parse(inicio.toString());
                    System.out.println("HORA DE INICIO " + horaIni);

                    horaFin = dateFormat.parse(fin.toString());
        ...
        if (!nutriologo.getNombre().equals("")) { //Ingresa solo si el campo nombre no esta vacío
            ...
            if (nutriologo.getNombre().matches("[a-zA-ZñÑáéíóúÁÉÍÓÚ\\s]+")) { //la letra s simbolisa espacio
        ...
        if (!nutriologo.getAp_uno().equals("")) { // Ingresa solo si el primer apellido no esta vacío
            //valida que solo se encuentren caracteres alfabeticos minusculos, mayusculos acentos y ñ Ñ ademas de permitir espacios, prohibe la entreda de digitos y caracteres especiales
            if (nutriologo.getAp_uno().matches("[a-zA-ZñÑáéíóúÁÉÍÓÚ\\s]+")) { //la letra s simbolisa espacio
        ...
        if (!nutriologo.getAp_dos().equals("")) { //Ingresa solo si el segundo apellido no se encuentra vació
            //valida que solo se encuentren caracteres alfabeticos minusculos, mayusculos acentos y ñ Ñ ademas de permitir espacios, prohibe la entreda de digitos y caracteres especiales
            if (nutriologo.getAp_dos().matches("[a-zA-ZñÑáéíóúÁÉÍÓÚ\\s]+")) { //la letra s simbolisa espacio
        ...
        if (!nutriologo.getContraseña().equals("")) {
            int invalido = 0;
            for (int i = 0; i < nutriologo.getContraseña().length(); i++) {
                if (nutriologo.getContraseña().charAt(i) == '\'') {
                    System.out.println("Se encontro una comilla");
                    invalido = 1;
                }
                if (nutriologo.getContraseña().charAt(i) == '\"') {
            ...
            if (invalido == 1) {
                ...
                if (!nutriologo.getContraseña2().equals("")) {
                    invalido = 0;
                    for (int i = 0; i < nutriologo.getContraseña2().length(); i++) {
                        if (nutriologo.getContraseña().charAt(i) == '\'') {
                            System.out.println("Se encontro una comilla");
                            invalido = 1;
                        }
                        if (nutriologo.getContraseña2().charAt(i) == '\"') {
                            System.out.println("Se encontro una comilla doble");
                            invalido = 1;
                        }
                        if (nutriologo.getContraseña2().charAt(i) == '|') {
                    ...
                    if (invalido == 1) {
                        ...
                        if (!nutriologo.getContraseña().equals("") && !nutriologo.getContraseña2().equals("")) {

                            if (nutriologo.getContraseña().length() > 7) { // LA CONTRASEÑA DEBE CONTENER MAS DE 7 CARACTERES
                                System.out.println("LA CONTRASEÑA 1 TIENE MAS DE 7 CARACTERES");
                                for (int i = 0; i <= nutriologo.getContraseña().length() - 1; i++) { // SE REALIZA UN RECORRIDO DESDE 0 HASTA LA LONGITUD DE LA CONTRASEÑA -1
                                    clave1 = nutriologo.getContraseña().charAt(i);            // SE ASIGNA EL CARACTER CON EL INDICE i 
                                ...
                                if (minuscula1 >= 1 && mayuscula1 >= 1 && numero1 >= 1) {      // SE VERIFICA QUE LA CONTRASEÑA CONTENGA AL MENOS UNA MINUSCULA, UNA MAYUSCULA Y UN NÚMERO
                                    ...
                                    if (nutriologo.getContraseña2().length() > 7) {           // LA CONTRASEÑA DEBE CONTENER MAS DE 7 CARACTERES

                                        System.out.println("LA CONTRASEÑA 2 TIENE MAS DE 7 CARACTERES");
                                        for (int i = 0; i <= nutriologo.getContraseña2().length() - 1; i++) {    // SE REALIZA UN RECORRIDO DESDE 0 HASTA LA LONGITUD DE LA CONTRASEÑA -1
                                            clave2 = nutriologo.getContraseña2().charAt(i);               // SE ASIGNA EL CARACTER CON EL INDICE i
                                        ...
                                        if (minuscula2 >= 1 && mayuscula2 >= 1 && numero2 >= 1) {    // SE VERIFICA QUE LA CONTRASEÑA CONTENGA AL MENOS UNA MINUSCULA, UNA MAYUSCULA Y UN NÚMERO
                                            if (nutriologo.getContraseña().equals(nutriologo.getContraseña2())) {  // SE COMPARA QUE LAS CONTRASEÑAS SEAN IGUALES
